...
disp("  ");
display("Computing summary statistics");
disp("  ");
...
if nsimqtrs > 100
    ...
    for itrack = 1:ntrack
        ...
        yy(:,:) = ysave(itrack,:,:);
        aaa = [track_names(itrack),'_mat = yy;'];
        aaa = strjoin(aaa);
        aaa = erase(aaa,' ');
        eval(aaa);
        zz = yy(101:nsimqtrs,:);
        ...
        ww = sort(ww);
        n = size(ww,2);
        sim_results(1,itrack) = mean(ww);
        sim_results(2,itrack) = median(ww);
        sim_results(3,itrack) = std(ww);
        sim_results(4,itrack) =  skewness(ww);
        sim_results(5,itrack) = ww(1,round(.025*n));
        sim_results(6,itrack) = ww(1,round(.05*n));
        sim_results(7,itrack) = ww(1,round(.15*n));
        sim_results(8,itrack) = ww(1,round(.85*n));
        sim_results(9,itrack) = ww(1,round(.95*n));
        sim_results(10,itrack) = ww(1,round(.975*n));
    end
...
if strcmp(uthresh_imposed,"yes") & uthresh < 9999
...
if strcmp(pithresh_imposed,"yes") & pithresh > -9999
    values = strvcat(values,'yes');
...
% ************************************************************
% Assemble stochastic sim parameters 
% ************************************************************
...
values = strvcat(values,sample_range(1));
titles = strvcat(titles,' ');
values = strvcat(values,sample_range(2));
stochsim_info.sample_range = sample_range;

titles = strvcat(titles,'  Pre-1983 wage-price shocks rescaled');
if find(strcmp(rescale_wpshocks,"yes"))
    values = strvcat(values,'yes');
else
    values = strvcat(values,'no');
end
...
if exist('res_drop')
    for i = 1:size(res_drop,2)
        ...
        values = strvcat(values,res_drop(i));
    end	
...
runtime = toc/60;
titles = strvcat(titles,'Stochastic simulation run time (minutes)   ');
...
if find(strcmp(elb_imposed,"yes"))
    ...
    mqconst = mean(z(z>0));
    titles = strvcat(titles,'  Percent of expected paths with binding ELB    ');
    values = strvcat(values,num2str(fconst,'%.2f')); 
    exppaths_info.elb.fconst = fconst;
    titles = strvcat(titles,'  Mean ELB quarters along constrained exp paths    ');
    ...
    titles = strvcat(titles,'  Percent of const exp paths with ELB violations    ');
    ...
    if size(z(z<-elb_epsi),1) > 0
        ...
        titles = strvcat(titles,'  Mean ELB violation along constr exp paths    ');
...
if strcmp(elb_imposed,"yes")
    ...
    values = strvcat(values,blanks(1));
    z = pathinfo(:,:,5);
    if strcmp(uthresh_imposed,"no") & strcmp(pithresh_imposed,"no")
        z0 = z(z~=-9999);
        z1 = z0((z0>0)&(z0<1000));
        z2 = z0(z0>1000);
        z3 = z0(z0==(-1000));
...
if strcmp(mprule,"aait") | strcmp(mprule,"adur")
    z1 = pathinfo(:,:,7);
    z2 = pathinfo(:,:,8);
...
if nsimqtrs > 100
    ...
    values = strvcat(values,num2str(loss,'%.2f'));
    ...
    if strcmp(elb_imposed,"yes")
        ...
        values = strvcat(values,num2str(elbfreq,'%.2f'));
   ...
   % Average duration of ELB episodes
    if strcmp(elb_imposed,"yes")
        ...
        titles = strvcat(titles,a);
        values = strvcat(values,num2str(melbdur,'%.2f'));
    ...
    if strcmp(ecfs_option,"yes")
        ...
        exppaths_info.ecfs.fshock_freq= fshockfreq;
    ...
    titles = strvcat(titles,a);
    values = strvcat(values,num2str(recess_freq,'%.2f'));
    ...
    titles = strvcat(titles,a);
    values = strvcat(values,num2str(mdur,'%.2f'));
    ...
    titles = strvcat(titles,a);
    values = strvcat(values,num2str(mdepth1,'%.2f'));
    longrun_info.mdepth1 = mdepth1;
    a = "  Average maximum unemployment gap in recessions     ";
    titles = strvcat(titles,a);
    values = strvcat(values,num2str(mdepth2,'%.2f'));
...
% ************************************************************
% Display long-run distributional statistics
...
if nsimqtrs > 100
    ...
    if exist('OCTAVE_VERSION')
        sumtab = sim_results;
        rowlabels = ['mean';'median';'std dev';'skew';'low 95';'low 90'; ...
                     'low 70';'high 70';'high 90';'high 95'];
        collabels = strvcat(track_names);
        prettyprint(sumtab,rowlabels,collabels);
    else
...
if strcmp(save_option,"full") | strcmp(save_option,"limited")
    fname = save_name;
    disp("  ");
    disp(horzcat("Writing output to: ",fname));
    save(fname,'runtime');
