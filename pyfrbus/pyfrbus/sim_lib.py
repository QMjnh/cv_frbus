...
def sim_plot(baseline: DataFrame, sim: DataFrame, start: str, end: str, file_name = 'plot.png') -> None:
    ...
    fig.tight_layout()
    plt.savefig(file_name) # added by minh
    plt.show()


def stochsim_plot(
    baseline: DataFrame, sims: List[DataFrame], start: str, end: str, image_link='png/image.png'
) -> None:
    # Pad with 25% history but not more than 6 or less than 2 qtrs
    back_pad = max(min(round((pd.Period(end) - pd.Period(start)).n / 4), 6), 2)
    plot_period = pd.period_range(pd.Period(start) - back_pad, end, freq="Q")
